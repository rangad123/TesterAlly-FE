...
const MemberRequirementType = () => {
  ...
  // State for editing
  const [editValues, setEditValues] = useState({ type_name: "", description: "" });
    ...
    const fetchRequirementTypes = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.message || "Failed to fetch requirement types");
        }

        const data = await response.json();
        setTypes(data);
      } catch (error) {
        setError(error.message || "Failed to fetch requirement types");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDelete = async (typeId) => {
    if (!window.confirm("Are you sure you want to delete this requirement type?")) return;
      ...
      <div className="flex-1 lg:ml-[300px] transition-all duration-300 lg:max-w-[calc(100%-300px)] sm:ml-[60px] sm:max-w-full">
        <div className="p-6">
          <div className="bg-white rounded-lg shadow mb-6">
            ...
            {loading ? (
              <div className="p-4 text-center">Loading requirement types...</div>
            ) : error ? (
              <div className="p-4 text-center text-red-500">{error}</div>
            ) : !selectedProject ? (
              <div className="p-4 text-center text-gray-500">
                Please select a project from the sidebar to view requirement types
              </div>
            ) : (
              <div className="p-4">
...
export default MemberRequirementType;